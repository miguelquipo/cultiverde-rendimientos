...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search-cedula']) && isset($_POST['search-producto']) && isset($_POST['insert-cantidad'])) {
    ...
    if ($stmtTrabajador === false) {
        ...
        header("Location: ../HTML/rendimientos.php?success=false&error=consulta_trabajador");
        exit();
    }
    ...
    if ($stmtUltimoRango === false) {
        ...
        header("Location: ../HTML/rendimientos.php?success=false&error=consulta_rango");
        exit();
    }
    ...
    for ($i = 0; $i < $cantidadVeces; $i++) {
        ...
        if ($stmtInsert === false) {
            ...
            header("Location: ../HTML/rendimientos.php?success=false&error=insert_fallido");
            exit();
        }
    }
